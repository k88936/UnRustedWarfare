    ...
    for (auto grid: this->gridsAcross) {
        for (auto tar: grid->objects) {
            ...
            if (positionDiff.lengthSquared() < (this->radius + tar->radius) * (this->radius + tar->radius)) {
                this->onOverlay(tar, positionDiff);
                tar->onOverlay(this, positionDiff);
...
void Object::onOverlay(Object *obj, QVector3D positionDiff) {
}
...
void Object::solveCollision(Object *obj1, Object *obj2, QVector3D positionDiff) {
    ...
    const float force = (2 - obj1->restitution - obj2->restitution) *(speedProjected + 1.0f) * obj1->mass * obj2->mass*(obj1->radius+obj2->radius) / (
                            (obj1->mass + obj2->mass) * (positionDiff.lengthSquared() +0.5f) * Game::deltaTime);;
